...
interface EntryRelationDraftFormProps {
    draft: EntryRelationDraft
    entries: EntryBrief[]
    categories: Category[]
    disabled?: boolean
    currentEntryId: string
    onChange: (draft: EntryRelationDraft) => void
    onDelete?: () => void
    onOpenEntry?: (entry: { id: string; title: string }) => void
}

export function EntryRelationDraftForm({
                                           draft,
                                           entries,
                                           categories,
                                           disabled = false,
    ...
    return (
        <div className="entry-relation-editor__item">
            <div className="entry-relation-editor__row">
                ...
                <div className="entry-relation-editor__field entry-relation-editor__field--direction">
...
export default function EntryRelationCreator({
    ...
    const handleDraftAdd = () => {
        onChange([...drafts, createEmptyRelationDraft()])
    }
                ...
                <div className="entry-relation-editor__list">
                    {drafts.map((draft, index) => (
                        <EntryRelationDraftForm
                            ...
                            disabled={disabled}
                            onChange={(nextDraft) => onChange(drafts.map((item, draftIndex) => (
                                draftIndex === index ? nextDraft : item
                            )))}
                            onDelete={() => handleDraftRemove(index)}
