...
/// 评估布局形状时假定的目标视口长宽比（宽/高）。
///
/// 前端用 fitView 铺满面板，缩放比是 `min(W/w, H/h)`，所以卡片在屏幕上的实际大小
/// 由 `max(w/aspect, h)` 决定——这个量才是"排版好不好看"的可度量代理。包围盒面积
/// 不区分横竖，按面积优化会把图揉成正方形：实测同一张 14 节点图，按面积收拢末端得到
/// 1209x1129（长宽比 1.07），按视口拟合得到 1514x899（1.68），后者卡片大 25%。
/// 设为 0 或负数退回面积评分（保留旧行为的开关）。
/// 调大：更愿意用宽度换高度（更扁）；调小：更接近正方形。
pub const VIEWPORT_ASPECT: f64 = 1.7;
/// 形状代价里非约束方向的收缩权重。
///
/// 纯拟合代价 `max(w/aspect, h)` 在非约束方向上梯度为零——宽度已经吃满时，把高度从 250
/// 压到 50 代价完全不变，末端收拢会在有余量时直接失效（会让"竖着挂在宽行旁边的双节点簇"
/// 永远不被摊平）。加一项两个归一化边长之和提供梯度，保证任何方向的收缩都有正收益。
/// 调大：更追求整体紧凑，可能牺牲拟合；调小（→0）：只认拟合，非约束方向放任不管。
pub const SHAPE_SLACK_WEIGHT: f64 = 0.125;
/// 末端节点候选位环形搜索的角度数（在锚点周围均匀取向）。
///
/// 原先只试锚点的正左和正右两点，且要求与原距离严格相等。这个候选集在真实布局里
/// 几乎必然撞到别的节点：实测 14 节点图上，无论怎么改评分和放开可动叶子数，
/// 收益都精确为零，直到把候选集换成环形才出现改善。
/// 调大：搜索更接近连续，收益略增而代价线性上升；调小：退化回稀疏采样。
pub const TERMINAL_TUCK_ANGLES: usize = 24;
/// 末端候选位的半径档数，在 `[TERMINAL_TUCK_MIN_RADIUS_FACTOR, 1]` 内均匀取值。
/// 调大：更细的半径搜索；设为 1 只试原距离。
pub const TERMINAL_TUCK_RADIUS_STEPS: usize = 4;
/// 末端候选位相对"原本到锚点距离"的最小半径系数。允许收得更近，因为力导给叶子的
/// 距离通常偏大；上限固定为 1，避免把叶子推得比原来更远。
/// 调小：允许叶子贴得更紧；设为 1 则只在原距离的圆周上搜索。
pub const TERMINAL_TUCK_MIN_RADIUS_FACTOR: f64 = 0.5;
