...
interface Props {
    ...
    onUnavailable: (label: string) => void; onGallery: () => void; onAttachDocuments: () => void; webSearchEnabled: boolean; onToggleWebSearch: () => void
...
export default function MobileAiComposer(p: Props) {
    return <>
        ...
        <MobileAnchoredMenu open={p.modelMenuOpen} onClose={p.onCloseModelMenu} anchorRef={p.modelMenuRef} containerRef={p.pageRef} ariaLabel={p.modelMenuMode === 'plugins' ? '切换 AI 插件' : '切换 AI 模型'} className="mobile-ai-model-menu" align="left" rightBoundaryRef={p.topActionsRef} rightBoundaryGap={8}>
