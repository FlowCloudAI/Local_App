 ...
 * 移动端设定检测页。
 *
 * 业务状态与桌面端共用 useWorldCheckController；本文件只负责手机上的单列报告、历史入口和浮层布局。
 */
...
export default function MobileWorldCheck({params, pop, push, startReportDiscussion}: Props) {
    ...
    return (
        <div className="mobile-page mobile-world-check">
            <MobilePageTopBar
                ...
                center={<span className="mobile-world-check__topbar-title">设定检测</span>}
                right={(
                    <MobileTopActionPill actions={[
                        {
                            key: 'refresh',
            ...
            <header className="mobile-world-check__intro">
                <span className="mobile-world-check__eyebrow">{projectName}</span>
                <h1>检查设定是否站得住</h1>
                <p>查矛盾、核对单个词条与世界规则的契合度，或评估公开发布风险。</p>
            </header>
            ...
            {task && (
                ...
                >
                    <span className="mobile-world-check__task-head">
                        <strong>{worldCheckKindLabel(task.checkKind)}</strong>
                        <span aria-live="polite">{taskStatusLabel(task)}</span>
                    </span>
                    <span className="mobile-world-check__task-activity">{task.currentActivity}</span>
                    <span className="mobile-world-check__task-phases" aria-label="检测阶段">
                        {WORLD_CHECK_PHASES.map((phase) => (
                            <i key={phase} data-status={getWorldCheckPhaseStatus(task, phase)}>
                                {PHASE_LABELS[phase]}
                            </i>
                        ))}
                    </span>
            ...
            {historyItems.length > 0 && (
                <details className="mobile-world-check__history">
                    <summary>
                        <span>历史报告</span>
                        <small>{historyItems.length} 份</small>
                    </summary>
                    <div className="mobile-world-check__history-list">
                        {historyItems.map((item) => (
                            <button
                                key={item.reportId}
                                type="button"
                                className={item.reportId === selectedReportId ? 'is-active' : ''}
                                onClick={() => setSelectedReportId(item.reportId)}
                            >
                                <span>
                                    <strong>{worldCheckKindLabel(item.checkKind)}</strong>
                                    <small>{formatWorldCheckDateTime(item.createdAt)}</small>
                                </span>
                                <em>{item.findingCount} 项问题</em>
                            </button>
                        ))}
                    </div>
                </details>
            ...
            {!activeRecord ? (
                <section className="mobile-world-check__launch" aria-labelledby="mobile-world-check-launch-title">
                    <div className="mobile-world-check__section-head">
                        <div>
                            <span className="mobile-world-check__eyebrow">新报告</span>
                            <h2 id="mobile-world-check-launch-title">选择检测方式</h2>
                        </div>
                        {historyLoading && <small>正在读取历史…</small>}
                    </div>
                ...
                <article className="mobile-world-check__report">
                    <header className="mobile-world-check__report-hero">
                        ...
                        <div className="mobile-world-check__report-actions">
                            ...
                            <Button type="button" variant="outline" onClick={() => void deleteReport(activeRecord.reportId)}>
                                删除
                            </Button>
                        </div>
                    </header>
            ...
            {task?.monitorOpen && (
                ...
                >
                    ...
                    <div className="mobile-world-check-task-panel__actions">
                        ...
                        {task.status === 'success' && (
                            <>
                                <Button type="button" variant="outline" onClick={openTaskReport}>查看报告</Button>
