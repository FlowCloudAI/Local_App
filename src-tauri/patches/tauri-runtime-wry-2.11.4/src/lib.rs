...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    // iOS 系统必带 WKWebView；主动查询 com.apple.WebKit bundle 在 iOS 26 模拟器中
    // 会于 Foundation 内触发 SIGTRAP。其余平台继续使用上游探测逻辑。
    #[cfg(target_os = "ios")]
    let webview_runtime_installed = true;
    #[cfg(not(target_os = "ios"))]
    let webview_runtime_installed = wry::webview_version().is_ok();
