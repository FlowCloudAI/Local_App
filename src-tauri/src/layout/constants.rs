...
/// 分量沿主轴/次轴展布之比超过该阈值时，触发长宽比兜底压实
/// （pathish 之外的第二触发条件，覆盖混合结构跑成瘦长条的情况）。
/// 调大：只有极端瘦长才压实；调小：接近方形的分量也会被压。
pub const ASPECT_COMPACTION_TRIGGER: f64 = 1.6;
/// 长宽比兜底压实的最大强度。
/// 调大：瘦长分量被更强地压向主轴；调小：兜底压实更温和。
pub const ASPECT_COMPACTION_MAX: f64 = 0.22;
...
/// 簇级布局的横向偏置（>1 偏横向铺开，=1 无方向偏好）。
/// 同时作用于初始同心环（拉成横向椭圆）与向心力（纵向收得更紧），
/// 让多簇分量倾向横向排布，缓解竖条式堆叠。
